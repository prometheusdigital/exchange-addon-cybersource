    ...
    function get_form_table( $form, $settings = array() ) {
        ...
        <div class="it-exchange-addon-settings it-exchange-cybersource-addon-settings">
            ...
            <?php
                ...
                $license = $exchangewp_cybersource_options['cybersource_license'];
                // var_dump($license);
                $exstatus = trim( get_option( 'exchange_cybersource_license_status' ) );
                // var_dump($exstatus);
             ?>
            <p>
              <label class="description" for="exchange_cybersource_license_key"><?php _e('Enter your license key'); ?></label>
              <!-- <input id="cybersource_license" name="it-exchange-add-on-cybersource-cybersource_license" type="text" value="<?php #esc_attr_e( $license ); ?>" /> -->
              <?php $form->add_text_box( 'cybersource_license' ); ?>
              <span>
                <?php if( $exstatus !== false && $exstatus == 'valid' ) { ?>
    							<span style="color:green;"><?php _e('active'); ?></span>
    							<?php wp_nonce_field( 'exchange_cybersource_nonce', 'exchange_cybersource_nonce' ); ?>
    							<input type="submit" class="button-secondary" name="exchange_cybersource_license_deactivate" value="<?php _e('Deactivate License'); ?>"/>
    						<?php } else {
    							wp_nonce_field( 'exchange_cybersource_nonce', 'exchange_cybersource_nonce' ); ?>
    							<input type="submit" class="button-secondary" name="exchange_cybersource_license_activate" value="<?php _e('Activate License'); ?>"/>
    						<?php } ?>
              </span>
            </p>
    ...
    */
    function save_settings() {
        ...
        // listen for our activate button to be clicked
      	if( isset( $_POST['exchange_cybersource_license_activate'] ) ) {

      		// run a quick security check
      	 	if( ! check_admin_referer( 'exchange_cybersource_nonce', 'exchange_cybersource_nonce' ) )
      			return; // get out if we didn't click the Activate button

      		// retrieve the license from the database
      		// $license = trim( get_option( 'exchange_cybersource_license_key' ) );
          $exchangewp_cybersource_options = get_option( 'it-storage-exchange_addon_cybersource' );
          $license = trim( $exchangewp_cybersource_options['cybersource_license'] );

      		// data to send in our API request
      		$api_params = array(
      			'edd_action' => 'activate_license',
      			'license'    => $license,
      			'item_name'  => urlencode( 'cybersource' ), // the name of our product in EDD
      			'url'        => home_url()
      		);
      			...
      			if ( false === $license_data->success ) {
      				...
      				switch( $license_data->error ) {
      					...
      					case 'item_name_mismatch' :
      						...
      						$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), 'cybersource' );
      						break;
      		...
      		if ( ! empty( $message ) ) {
      			$base_url = admin_url( 'admin.php?page=it-exchange-addons&add-on-settings=' . 'cybersource' );
            ...
            return;
      		...
      		update_option( 'exchange_cybersource_license_status', $license_data->license );
      		// wp_redirect( admin_url( 'admin.php?page=it-exchange-addons&add-on-settings=' . 'cybersource' ) );
      		// exit();
          return;
      	}
        ...
        // deactivate here
        // listen for our activate button to be clicked
      	if( isset( $_POST['exchange_cybersource_license_deactivate'] ) ) {
          ...
          $license = $exchangewp_cybersource_options['cybersource_license'];
            ...
            return;
      		...
      		if( $license_data->license == 'deactivated' ) {
      			delete_option( 'exchange_cybersource_license_status' );
      		}
