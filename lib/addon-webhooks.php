 ...
 * @return array
*/
function it_exchange_cybersource_addon_register_webhook_key() {
    $key   = 'cybersource';
    $param = apply_filters( 'it_exchange_cybersource_addon_webhook', 'it_exchange_cybersource' );
    it_exchange_register_webhook( $key, $param );
}
 ...
 */
function it_exchange_cybersource_addon_process_webhook( $request ) {
        ...
        try {
            ...
            switch( $stripe_event->type ) :
                ...
                case 'charge.succeeded' :
                    it_exchange_cybersource_addon_update_transaction_status( $stripe_object->id, 'succeeded' );
                    break;
                case 'charge.failed' :
                    it_exchange_cybersource_addon_update_transaction_status( $stripe_object->id, 'failed' );
                    break;
                case 'charge.refunded' :
                    if ( $stripe_object->refunded )
                        it_exchange_cybersource_addon_update_transaction_status( $stripe_object->id, 'refunded' );
                    else
                        it_exchange_cybersource_addon_update_transaction_status( $stripe_object->id, 'partial-refund' );

                    it_exchange_cybersource_addon_add_refund_to_transaction( $stripe_object->id, $stripe_object->amount_refunded );

                    break;
                case 'charge.dispute.created' :
                case 'charge.dispute.updated' :
                case 'charge.dispute.closed' :
                    it_exchange_cybersource_addon_update_transaction_status( $stripe_object->charge, $stripe_object->status );
                    break;
                case 'customer.deleted' :
                    it_exchange_cybersource_addon_delete_cybersource_id_from_customer( $stripe_object->id );
                    break;
...
add_action( 'it_exchange_webhook_it_exchange_cybersource', 'it_exchange_cybersource_addon_process_webhook' );
